import Rating from '../../assets/icons/MovieCardRating';
...
const MovieCardInfo = ({ movie, dropdown }: IMovieCardInfo) => {
  return (
    <div className={`${styles.movieCardInfo} ${dropdown ? styles.dropdownMovieCardInfo : ''}`}>
      <div
      ...
      {movie.genres.map((genre, index) => (
        <span key={index} className={`${styles.genre} ${dropdown ? styles.dropdownGenre : ''}`}>
          {genre}
        </span>
      ))}

      <span className={`${styles.duration} ${dropdown ? styles.dropdownDuration : ''}`}>
